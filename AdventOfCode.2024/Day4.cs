namespace AdventOfCode
{
	...
	{
		readonly string xPattern = @"X";
		readonly string mPattern = @"M";
		readonly string aPattern = @"A";
		readonly string sPattern = @"S";
		readonly List<XMAS> xList = new List<XMAS>();
		readonly List<XMAS> mList = new List<XMAS>();
		readonly List<XMAS> aList = new List<XMAS>();
		readonly List<XMAS> sList = new List<XMAS>();
		...
		public Day4(string input) : base(input)
		{
			...
			while (!this.reader.EndOfStream)
			{
				...
				foreach (Match match in xMatches) xList.Add(new X(match.Index, y));
				foreach (Match match in mMatches) mList.Add(new M(match.Index, y));
				foreach (Match match in aMatches) aList.Add(new A(match.Index, y));
				foreach (Match match in sMatches) sList.Add(new S(match.Index, y));
		...
		public void Task2()
		{
			...
			foreach (var a in aList)
			{
				...
				XMAS SWM = null!;
				if (NES != null) SWM = a.FindNext(mList, Direction.SW);
				if (SWM != null) xmas++;
				
				// m on the north east
				var NEM = a.FindNext(mList, Direction.NE);
				XMAS SWS = null!;
				if (NEM != null) SWS = a.FindNext(sList, Direction.SW);
				if (SWS != null) xmas++;

				// s on the south east
				var SES = a.FindNext(sList, Direction.SE);
				XMAS NWM = null!;
				if (SES != null) NWM = a.FindNext(mList, Direction.NW);
				if (NWM != null) xmas++;

				// m on the south east
				var SEM = a.FindNext(mList, Direction.SE);
				XMAS NWS = null!;
				if (SEM != null) NWS = a.FindNext(sList, Direction.NW);
				if (NWS != null) xmas++;
